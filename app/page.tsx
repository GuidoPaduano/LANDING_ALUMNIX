  ...
  return (
    <section id="comunidad" className="overflow-hidden bg-slate-950 py-20 text-white sm:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8"><div className="grid gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">{roles.map((item, index) => <article key={item.role} className={`rounded-3xl border border-white/10 bg-white/[0.06] p-6 backdrop-blur transition hover:-translate-y-1 hover:bg-white/[0.1] lg:col-span-2${index === 3 ? " lg:col-start-2" : ""}`}><span className="grid size-11 place-items-center rounded-2xl bg-blue-500/15 text-cyan-300"><item.icon className="size-5" /></span><h3 className="mt-6 text-lg font-bold">{item.role}</h3><p className="mt-2 leading-6 text-slate-400">{item.text}</p></article>)}</div>
      </div></div>
    </section>
  )
}
